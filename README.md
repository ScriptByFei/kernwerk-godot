...
| 10 | Echte Walk-Animationen: 4-Frame-Top-Down-Watschel-Gang (Wippen ±3°, Y-Bob, pulsierender Boden-Schatten) für Grunt/Runner/Tank | ✅ |
| 11 | Prozedural neu gezeichnete Figuren: 3/4-Ansicht mit sichtbaren Beinen + Füßen (Archero-Stil, Kenney-Paletten), echter Bein-Schwung im Walk-Zyklus — keine „Flugzeug“-Silhouetten mehr | ✅ |
...
- **Phase 10 (02.09.)** — Echte Walk-Animationen live (Top-Down-Watschel-Gang: Wippen ±3°, Bob, Schatten-Puls, 6 fps). **Nächster iPhone-Playtest: wirkt der Gang natürlich oder zu wackelig?** (Tuning: `WALK_FRAME_SPECS` in `tools/sprite_pipeline.py` + Speed in `enemy.gd`)
- **Phase 11 (02.09.)** — Figuren komplett neu gezeichnet (3/4-Ansicht, sichtbare Beine + Füße, Archero-Stil, Kenney-Paletten) — kein „Flugzeug“-Look mehr. **Nächster iPhone-Playtest: passt der neue Figuren-Look?** (Geometrie/Paletten in `UNIT_SPECS` + `render_character_frame` in `tools/sprite_pipeline.py`)
