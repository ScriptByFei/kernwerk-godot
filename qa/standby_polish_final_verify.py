#!/usr/bin/env python3
"""Final-correction evidence runner. No deployment. Run from repository root.
Exports the Web release to a NEW path /tmp/kernwerk-standby-web-final/ with
XDG_DATA_HOME=/home/masgi_bot/.local/share so export templates resolve.
...
TARGET = Path('/tmp/kernwerk-standby-web-final')
ENV = dict(os.environ)
...
if __name__ == '__main__':
    ...
    run('web_export_standby_' + suffix,
        ['godot4', '--headless', '--audio-driver', 'Dummy', '--path', '.',
         '--export-release', 'Web', str(TARGET / 'index.html')])
    artifacts = {}
