...
extends Control
## Ergebnisanzeige nach dem Absturz: zeigt den erreichten Score, den Bestwert
## der Sitzung und ob ein neuer Bestwert aufgestellt wurde. Der Neustart
## passiert nur auf Wunsch (NEU STARTEN), nicht automatisch.
##
...
func _apply_layout() -> void:
	...
	_score_font = maxi(roundi(SCORE_FONT * _unit), ceili(20.0 / pixel_scale))
	...
	var panel_h := (
		...
		+ RECORD_TO_SCORE * _unit + score_h + SCORE_TO_BEST * _unit + best_h
		+ BEST_TO_ROW * _unit + row_height
	)
	_panel = Rect2(
	...
	cursor += record_h + RECORD_TO_SCORE * _unit
	...
	cursor += best_h + BEST_TO_ROW * _unit
