...
func _run() -> void:
	...
	for offset in [0.0, 90.0]:
		...
		_check(game.resonance.charges == 3 and game.resonance.overload_count == 0, "third charge remains READY")
		_check(not game.jumper.is_overloaded(), "READY does not play discharge effects")
		_check(game.try_arm_overload(), "explicit activation succeeds")
		game.jumper._resolve_landing(platform, false, platform.position.x + offset)
		_check(-game.jumper.velocity.y >= speeds[1] * 1.10, "chosen overload launch at least 10 percent faster than second")
		...
		_check(game.resonance.charges == 0 and game.resonance.overload_count == 1, "chosen next bounce discharges once")
